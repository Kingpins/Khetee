...
def diesesdetection_main():
    st.set_option('deprecation.showfileUploaderEncoding', False)
    st.markdown('<style>body{-webkit-app-region: drag;}</style>', unsafe_allow_html=True)
    st.write('<style>div.row-widget.stRadio > div{flex-direction:row;} </style>', unsafe_allow_html=True)
    html_temp = """
    <div>
    <h1 style="color:MEDIUMSEAGREEN;text-align:left;"> Plant Disease Detection and Classification 🌳 </h1>
    </div>
    """
    st.markdown(html_temp, unsafe_allow_html=True)
    activities = ["About" ,"Plant Disease"]
    choice = st.radio("Select Activty",activities)
    if choice =='About':
        intro_markdown = read_markdown_file("./doc/disease.md")
        st.markdown(intro_markdown, unsafe_allow_html=True)
    if choice =='Plant Disease':
        enhance_type = st.radio("Type",["Detection","Classification","Treatment"])
        ...
        if enhance_type == 'Classification':
